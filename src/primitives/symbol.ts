...
export class TyrunSymbolSchema extends TyrunBaseSchema<symbol, symbol, TyrunSymbolConfig> implements TyrunSymbolType {
  ...
  public override parse(input: unknown): symbol {
    try {
      if (input === undefined && this.__config.default !== undefined) return this.runDefault()

      const preprocessed = this.runPreprocessors(input)

      if (typeof preprocessed !== 'symbol') throw new TyrunError([this.buildIssue(CODES.BASE.TYPE, ERRORS.BASE.TYPE, [], this.__config.error)])
  ...
  public override async parseAsync(input: unknown): Promise<symbol> {
    try {
  ...
  public override safeParse(input: unknown): Result<symbol> {
    try {
  ...
  public override async safeParseAsync(input: unknown): Promise<Result<symbol>> {
    try {
  ...
  public override clone(): TyrunSymbolSchema {
    return new TyrunSymbolSchema(this.__config)
  }
}
