...
import type { Result, TyrunBaseConfig, TyrunNullConfig, TyrunNullType } from '../types'

export class TyrunNullSchema extends TyrunBaseSchema<null, null, TyrunNullConfig> implements TyrunNullType {
  public override readonly type: 'null' = 'null' as const

  constructor(config: TyrunBaseConfig<TyrunNullConfig, null, null>) {
    super(config)
  }

  public override parse(input: unknown): null {
    try {
      if (input === undefined && this.__config.default !== undefined) return this.runDefault()

      const preprocessed = this.runPreprocessors(input)

      if (preprocessed !== null) throw new TyrunError([this.buildIssue(CODES.BASE.TYPE, ERRORS.BASE.TYPE, [], this.__config.error)])
  ...
  public override async parseAsync(input: unknown): Promise<null> {
    try {
  ...
  public override safeParse(input: unknown): Result<null> {
    try {
  ...
  public override async safeParseAsync(input: unknown): Promise<Result<null>> {
    try {
  ...
  public override clone(): TyrunNullSchema {
    return new TyrunNullSchema(this.__config)
  }
}
